...
type DistributionData = {
  rating: { bin: string; count: number }[];
  size: { bin: string; count: number }[];
  installs: { bin: string; count: number }[];
};
...
export default function Home() {
    ...
    async function loadDashboard() {
      try {
        const [
          ...
          api<CategoryRow[]>("/api/analytics/categories"),
          api<Outlier[]>("/api/analytics/outliers"),
          api<PipelineData>("/api/analytics/pipeline"),
          api<QualityData>("/api/analytics/data-quality"),
          api<DistributionData>("/api/analytics/distributions"),
          api<{ items: string[] }>("/api/analytics/insights"),
        ]);
  ...
  useEffect(() => {
    if (page !== "Hexbin Explorer") return;
  ...
  const filteredOutliers = useMemo(() => {
    return outs.filter((item) => {
      ...
      const matchesType =
        outlierFilter === "ALL" ||
        item.outlier_type.toUpperCase() === outlierFilter;
  ...
  const title =
    page === "Overview" ? "Mobile App Intelligence" : page;

  if (page === "Landing") {
    return <Landing onExplore={() => setPage("Overview")} />;
  }
                ...
                <div className="card p-5 mt-5">
                  <div className="flex flex-wrap gap-4 items-center justify-between mb-5">
                    <div>
                      ...
                      <p className="muted text-sm mt-1">
                        Install density across the filtered application
                        ecosystem.
                      </p>
                    </div>
                  ...
                  <div className="grid md:grid-cols-3 gap-4 mt-5">
                    ...
                    <MiniInfo
                      ...
                      title="Applications"
                      value={hex.count?.toLocaleString() || "0"}
                    />
                  </div>
                </div>
              ) : (
                <BlockedAnalysis status={status} />
              )}
            </>
          )}
  ...
  return (
    <>
      ...
      <div className="grid sm:grid-cols-2 xl:grid-cols-6 gap-4 mt-5">
        <Kpi
          label="Total Apps"
          value={summary.raw_rows?.toLocaleString()}
          sub="Original dataset"
        />

        <Kpi
          label="Pre-Subjectivity"
          value={summary.pre_subjectivity_rows?.toLocaleString()}
        ...
        <Kpi
          ...
          value={
            ...
            summary.avg_installs !== undefined
              ? summary.avg_installs.toLocaleString(undefined, {
        ...
        <Kpi
          label="Outliers"
          value={summary.outliers?.toLocaleString()}
      ...
      <div className="grid xl:grid-cols-[1.4fr_.9fr] gap-5 mt-5">
        <div className="card p-6">
          <div className="flex items-start justify-between gap-5">
            <div>
              ...
              <p className="muted mt-2 max-w-2xl">
                Explore app ratings, size, installs, reviews, category
                behavior and category-level statistical outliers using
                the supplied Google Play dataset.
              </p>
            </div>
          ...
          <div className="grid md:grid-cols-3 gap-3 mt-6">
            ...
            <InsightStat
              label="Cleaned rows"
              value={quality?.valid_cleaned_rows ?? "—"}
            />
        ...
        <div className="card p-6">
          <div className="flex items-center gap-2">
            <Info size={18} />
            <h2 className="font-bold">Data integrity</h2>
          </div>

          <p className="muted text-sm mt-3">
            No synthetic KPI or subjectivity values are generated.
            The supplied CSV does not contain review text or a
            subjectivity column.
          </p>
...
function AnalyticsPage({
  ...
  const categoryOption = {
    ...
    xAxis: {
      type: "category",
      data: categories.map((x) => translateCategory(x.category)),
      axisLabel: {
        rotate: 25,
      },
    },
  ...
  const ratingOption = {
    ...
    xAxis: {
      type: "category",
      data:
        distributions?.rating.map((x) => x.bin) || [],
    },
    yAxis: {
      type: "value",
    },
    series: [
      {
        type: "bar",
        data:
          distributions?.rating.map((x) => x.count) || [],
        barMaxWidth: 28,
        itemStyle: {
  ...
  const sizeOption = {
    ...
    xAxis: {
      type: "category",
      data:
        distributions?.size.map((x) => x.bin) || [],
    },
    yAxis: {
      type: "value",
    },
    series: [
      {
        type: "bar",
        data:
          distributions?.size.map((x) => x.count) || [],
        barMaxWidth: 28,
        itemStyle: {
          borderRadius: [6, 6, 0, 0],
        },
        ...
        <div className="flex flex-wrap items-start justify-between gap-5">
          <div>
            ...
            <p className="muted mt-2">
              Statistical exploration of the supplied Google Play
              dataset.
            </p>
          </div>
        ...
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <p className="muted text-sm mt-2">
              The supplied dataset contains no review text and no
              Subjectivity field. Therefore Subjectivity &gt; 0.5
              cannot be truthfully applied. The application does not
              invent NLP values.
            </p>
          </div>
        </div>
    ...
    <div className="card p-5">
      <div className="flex flex-wrap justify-between gap-4">
        <div>
          ...
          <p className="muted text-sm mt-1">
            Category-level IQR detection for Size and Rating.
          </p>
        </div>
      ...
      <div className="overflow-auto mt-6">
        <table className="w-full text-sm">
          ...
          <tbody>
            {outliers.slice(0, 250).map((item, index) => (
              <tr
        ...
        {!outliers.length && (
          <div className="py-16 text-center">
            ...
            <p className="muted text-sm mt-1">
              No category-level IQR observations match the current
              search/filter.
            </p>
          </div>
        )}
  ...
  return (
    <div className="space-y-5">
      <div className="card p-6">
        ...
        <p className="muted mt-2">
          Compare the nine requested categories using actual
        ...
        {categories.map((item) => (
          ...
          >
            ...
            <div className="grid grid-cols-2 gap-3 mt-5">
              <Stat label="Rating" value={item.avg_rating.toFixed(2)} />
              <Stat
                label="Size"
              ...
              <Stat
                label="Installs"
                value={item.avg_installs.toLocaleString(undefined, {
                  maximumFractionDigits: 0,
                })}
              />
              ...
              <Stat
                label="Reviews"
                value={item.avg_reviews.toLocaleString(undefined, {
                  maximumFractionDigits: 0,
                })}
              />
            </div>
            ...
            <div className="mt-4 text-xs muted">
              {item.outliers} category-level IQR outliers
            </div>
          </motion.div>
        ))}
      </div>
...
function PipelinePage({
  ...
  return (
    <div className="card p-6">
      ...
      <p className="muted mt-2">
        Actual record counts at each processing stage.
      </p>

      <div className="mt-8 space-y-4">
        {pipeline?.stages.map((stage, index) => {
          const available = stage.count !== null;
...
function QualityPage({
  ...
  const missingOption = {
    ...
    yAxis: {
      type: "category",
      data: missingEntries.map(([key]) => key),
    },
    series: [
      {
        type: "bar",
        data: missingEntries.map(([, value]) => value),
        barMaxWidth: 30,
        itemStyle: {
  ...
  const sizeOption = {
    ...
    xAxis: {
      type: "category",
      data: distributions?.size.map((x) => x.bin) || [],
    },
    yAxis: {
      type: "value",
    },
    ...
    series: [
      {
        type: "bar",
        data: distributions?.size.map((x) => x.count) || [],
        barMaxWidth: 30,
        itemStyle: {
          borderRadius: [6, 6, 0, 0],
        },
  ...
  return (
    <div className="space-y-5">
      <div className="card p-6">
        ...
        <p className="muted mt-2">
          Data-quality metrics calculated directly from the supplied
          CSV.
        </p>
      </div>
      ...
      <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <Kpi
          label="Total Rows"
          value={quality?.total_rows?.toLocaleString()}
        />
        ...
        <Kpi
          ...
          value={quality?.invalid_values?.toLocaleString()}
        />

        <Kpi
          label="Cleaned Rows"
          value={quality?.valid_cleaned_rows?.toLocaleString()}
        />
      </div>
  ...
  return (
    <div className="space-y-5">
      <div className="card p-6">
        ...
        <p className="muted mt-2">
          Download analytical outputs generated from the real
          dataset.
        </p>
      </div>
      ...
      <div className="grid md:grid-cols-3 gap-4">
        <ExportCard
          ...
          onClick={() => download("/api/reports/processed-csv")}
        />

        <ExportCard
          title="Outlier CSV"
          description="Download category-level IQR outliers."
          onClick={() => download("/api/reports/outliers-csv")}
        />

        <ExportCard
          title="Pipeline CSV"
          description="Download filtering-stage counts."
          onClick={() => download("/api/reports/pipeline-csv")}
        />
      </div>
    </div>
  );
    ...
    <div className="card p-6">
      ...
      <div className="grid md:grid-cols-2 gap-4 mt-6">
        ...
        >
          ...
          <p className="muted text-sm mt-1">
            5:00 PM–7:00 PM IST for live hexbin analytics.
          </p>
        </div>
      </div>
    </div>
        ...
        <section className="grid lg:grid-cols-2 gap-12 items-center min-h-[75vh]">
          <div>
            ...
            <p className="text-lg muted mt-7 max-w-xl">
              Explore application size, ratings, installs,
              reviews and statistical anomalies through an
              interactive analytics platform built around real
              Google Play data.
            </p>
  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={pink ? { color: "var(--game)" } : undefined}
      >
        {value}
      </div>
    </div>
  ...
  return (
    <div className="card p-5">
      <div
        ...
        style={{ background: "rgba(99,91,255,.12)" }}
      >
        {icon}
      </div>
